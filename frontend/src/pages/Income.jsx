...
import { Plus, Search, Trash2, Edit3, Calendar, DollarSign, X, AlertCircle, CheckCircle2, TrendingUp, ChevronDown, Download, Upload, Filter, ArrowUpDown } from 'lucide-react';
...
const Income = () => {
  ...
  const loadData = async () => {
    try {
      ...
      setCategories(catRes.data.filter(c => c.type === 'income' || c.type === 'both'));
  ...
  const handleSubmit = async (e) => {
    ...
    if (!accountId) {
      setError('Please configure and select an account to credit this income.');
      return;
    }
    ...
    try {
      if (editingIncome) {
        await api.put(`/transactions/${editingIncome.id}`, payload);
        setSuccess('Income transaction updated successfully.');
      } else {
        await api.post('/transactions/', payload);
        setSuccess('Income transaction created successfully.');
      }
  ...
  const handleDelete = async (txId) => {
    if (!window.confirm('Are you sure you want to delete this income transaction? This will restore the account balance.')) return;
    setError('');
    setSuccess('');

    try {
      await api.delete(`/transactions/${txId}`);
      setSuccess('Income transaction deleted and account balance restored.');
      loadData();
    } catch (err) {
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedIds(incomes.map(inc => inc.id));
    } else {
      setSelectedIds([]);
    }
  };

  const handleSelectRow = (e, incId) => {
    if (e.target.checked) {
      setSelectedIds([...selectedIds, incId]);
    } else {
      setSelectedIds(selectedIds.filter(id => id !== incId));
    }
  };
  ...
  const handleExport = async (format) => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `Income_Export_${new Date().toISOString().split('T')[0]}.${format === 'excel' ? 'xlsx' : 'csv'}`);
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          <h2 className="text-3xl font-bold text-dark-50 tracking-tight">Income</h2>
          <p className="text-dark-400 text-sm mt-1">Track company cash inflows, invoices, and sales earnings.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 rounded-xl bg-dark-900/20 border border-dark-850">
          <TrendingUp className="w-12 h-12 text-dark-500 mx-auto mb-4 animate-bounce" />
          <h3 className="text-base font-semibold text-dark-250">No Income Recorded</h3>
          <p className="text-xs text-dark-450 mt-1 max-w-md mx-auto leading-relaxed">
            Record cash inflows or import CSV tables to see income grouped by categories and accounts.
          </p>
        </div>
      ) : (
        <Card className="p-0 overflow-hidden border border-dark-850">
                ...
                {incomes.map((exp) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4 text-right font-bold text-green-400 whitespace-nowrap">
          ...
          <div className="px-4 py-3.5 bg-dark-950/50 border-t border-dark-850 flex justify-between items-center text-xs select-none">
            <span className="text-dark-400 font-medium">
              Showing {incomes.length} of {totalCount} records
            </span>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-dark-900 border border-dark-800 rounded-3xl w-full max-w-lg shadow-2xl p-6 relative">
            ...
            <h3 className="text-lg font-bold text-dark-50 mb-1">
              {editingIncome ? 'Modify Income Record' : 'Record Company Inflow'}
            </h3>
            <p className="text-xs text-dark-400 mb-6">Reconciles cash inflows against your user account ledger balances.</p>

            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-dark-400 uppercase tracking-wider mb-2">Description / Client Name</label>
                <input
                  type="text"
                  required
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  placeholder="e.g. Acme Corp Consulting Fee"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-dark-400 uppercase tracking-wider mb-2">Credit To Account</label>
                  <select
                    required
                    value={accountId}
