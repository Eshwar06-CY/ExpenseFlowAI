...
def test_stream():
    ...
    logger.info("==================================================")
    logger.info("GEMINI DIAGNOSTIC TEST: generate_content_stream")
    ...
    prompt = "Write a 1-sentence confirmation that Gemini generate_content_stream is working."

    try:
        logger.info("Executing client.models.generate_content_stream(model='%s')...", model)
        response_stream = client.models.generate_content_stream(
            model=model,
            contents=prompt,
            config=config,
        )
        logger.info("Stream opened successfully! Iterating response chunks...")
        accumulated_text = ""
        chunk_count = 0
        for chunk in response_stream:
            if chunk and chunk.text:
                ...
                logger.info("Chunk #%d received: %r", chunk_count, chunk.text)
        logger.info("Response Status  : SUCCESS 200 OK")
        logger.info("Total Chunks     : %d", chunk_count)
        logger.info("Accumulated Text : %s", accumulated_text)
